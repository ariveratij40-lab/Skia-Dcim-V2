...
2. `prepare_fixtures.sh` validates and pre-creates an absolute manifest destination outside the repository with mode `0600`, then invokes `prepare_fixtures.sql`. The SQL requires separate write and profile-baseline approvals, uses deterministic UUIDs, creates fixture V1 in a transaction, never alters schema/RLS, and exports exact IDs through psql `\g`. An export open/write failure stops psql before `COMMIT` and rolls back the transaction. The wrapper removes the incomplete manifest when psql fails. It creates exactly three tenant-local roles named `operator`, one per TEST tenant. ADMIN, OPERATOR and MULTI-BRANCH are logical personas sharing that role; ADMIN/MULTI receive two branch mappings and OPERATOR one. Each role receives only normative `dcim:view`, classified `NO ENFORCED`.
...
The preparation wrapper requires an absolute external path, rejects repository paths, symlink destinations and pre-existing files, and pre-creates the manifest with mode `0600`. The SQL exports `table_name, exact_id, logical_alias` through psql `\g` before `COMMIT`; `ON_ERROR_STOP` makes an export failure abort the session and transaction. After an HTTP campaign, the optional read-only session capture in `verify_fixtures.sql` exports exact session IDs (never tokens) to a second external file; an authorized operator must merge those rows into the rollback manifest. Before rollback, the operator must compute SHA-256 externally and retain the consolidated manifest until rollback is complete. Only the checksum and a nonsensitive count summary may be versioned as evidence.

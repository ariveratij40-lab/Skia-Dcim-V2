...
1. `preflight.sh` is read-only and checks the explicit staging environment, authorized host expression, expected database, exact application SHA covered by the PHASE-003 role-name trace, required schema objects, canonical/noncanonical `TEST-*` collisions and compatibility of the neutral runtime role `operator`. It requires exactly the normative `dcim:view` catalog row and reports it as `NO ENFORCED`; it never infers or clones permissions from real roles.
...
- the approved application SHA and neutral role name for preflight;
...
- The application SHA must equal the SHA covered by the approved PHASE-003 role-name trace. Any code change blocks preparation until role-name behavior is reviewed again.

import { Check, Circle, Lock, RefreshCw, X, Building2, MapPin, Database, Grid3x3 } from 'lucide-react';
...
export default function InfrastructureReadinessWizard({ data, loading, error, darkMode, onClose, onRetry, onAction }: Props) {
  ...
  return (
    <div role="dialog" aria-modal="true" aria-labelledby="readiness-title" style={{ position:'fixed', inset:0, zIndex:90, display:'flex', alignItems:'center', justifyContent:'center', padding:16, background:'rgba(15,23,42,.48)', backdropFilter:'blur(4px)' }}>
      <section style={{ width:'min(620px,100%)', maxHeight:'90vh', overflowY:'auto', borderRadius:18, border:`1px solid ${border}`, background:surface, color:text, boxShadow:'0 24px 70px rgba(15,23,42,.3)' }}>
        <header style={{ display:'flex', justifyContent:'space-between', gap:16, padding:'20px 22px 14px', borderBottom:`1px solid ${border}` }}>
          <div><h2 id="readiness-title" style={{ margin:0, fontSize:20 }}>Configure su infraestructura</h2><p style={{ margin:'6px 0 0', color:muted, fontSize:13 }}>SKIA necesita conocer la estructura física antes de registrar equipos.</p></div>
          <button aria-label="Cerrar" onClick={onClose} style={{ border:0, background:'transparent', color:muted, cursor:'pointer', height:32 }}><X size={18}/></button>
        </header>

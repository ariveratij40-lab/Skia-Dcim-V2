...
- Result: **RECONCILED** by the canonical import decision and migrations 017/018
...
| `migrations/009_add_unique_branches_constraint.sql` | historical compatibility only | excluded; guarded equivalent is migration 017 |
...
| `backend/migrations/010_enterprise_import_schema.sql` | superseded/duplicate/conflicting | excluded from the canonical clean-bootstrap path |
...
| embedded `009_capex_schema` | production schema required | materialized by forward-only migration 018 |
...
| embedded `013_racks_mdf_idf_id` | production schema required | materialized by forward-only migration 018 |
| embedded `014_cert_evaluations` | production schema required | materialized by forward-only migration 018 |

## Decision

`003_rbac_validation_data.sql` remains preserved but excluded. The executable
manifest is approved by the canonical import decision and ephemeral validation.

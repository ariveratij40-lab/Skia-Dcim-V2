...
- Nomenclature readiness is asset-type scoped. The summary is `configured` when active MDF and IDF rules are both visible for the session tenant, `partial` when exactly one type is configured, and `unavailable` when neither is configured. The per-type detail is the UX authority: each configured type has its own rule-derived Branch preview and only that type exposes its creation action. With no active rule, the only CTA is the existing nomenclature catalog; SKIA does not launch a creation flow that must end in `422 nomenclature_required`.
- The readiness preview remains educational and read-only: it uses neutral Site, Internal Area and Placement placeholders, never reads or reserves either `nomenclature_branch_counters` or `nomenclature_counters`, and never exposes `last_seq`. The real sequence remains transactional at asset creation.

...
docker exec "$container" sed -i -e '/034_canonical_infrastructure_housing_governance.sql/d' -e '/035_remove_legacy_rack_authorities.sql/d' /repo/ops/phase010/bootstrap.manifest
...
[[ "$(docker exec "$container" psql -X -U postgres -d skia_030_rollback -Atqc "SELECT count(*) FROM production_bootstrap_migrations")" == 22 ]]

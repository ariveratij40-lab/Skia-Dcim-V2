...
[[ "$(q skia_prod 'SELECT count(*) FROM production_bootstrap_migrations')" == 29 ]]
...
docker exec "$container" sh -c "cp -a /repo /repo-pre037 && sed -i '/037_nomenclature_v2_enforcement_audit_writer.sql/d' /repo-pre037/ops/phase010/bootstrap.manifest"
...
printf '%s\n' \
  ...
  'PRODUCTION_PRESET_SEED_COUNT=0' 'V2_CATALOG_ACTIVE=NO' 'LEDGER_COUNT=29' \

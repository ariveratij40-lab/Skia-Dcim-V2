...
roles="$(docker exec -e PGPASSWORD="$POSTGRES_BOOTSTRAP_PASSWORD" skia_postgres_prod \
  ...
  "SELECT string_agg(rolname||'|super='||rolsuper||'|bypass='||rolbypassrls||'|createdb='||rolcreatedb||'|createrole='||rolcreaterole,',' ORDER BY rolname) FROM pg_roles WHERE rolname IN ('skia_migrator','skia_runtime')")"
...
[[ "$schema_hash" == 61bdcf58f437c5ab4d5c48ad48b14c9ba1af3a0439eb7a04f22da9d4817f3792 ]]
[[ "$ledger" == 10 ]]

...
SELECT 'GRANT SELECT, INSERT ON TABLE public.floors, public.zones TO skia_runtime'
WHERE to_regclass('public.internal_areas') IS NOT NULL \gexec
SELECT 'GRANT SELECT ON TABLE public.technical_rooms TO skia_runtime'
WHERE to_regclass('public.internal_areas') IS NOT NULL \gexec
...
SELECT 'REVOKE ALL ON FUNCTION public.validate_import_row_for_commit(BIGINT,BIGINT,UUID,UUID), public.claim_import_row_for_commit(BIGINT,BIGINT,UUID,UUID,TEXT), public.complete_import_row_commit(BIGINT,BIGINT,UUID,UUID,TEXT,UUID), public.fail_import_row_commit(BIGINT,BIGINT,UUID,UUID,TEXT), public.recompute_inventory_import_state(BIGINT,UUID,UUID) FROM PUBLIC'
...
WHERE to_regprocedure('public.recompute_inventory_import_state(bigint,uuid,uuid)') IS NOT NULL \gexec
SELECT 'REVOKE ALL ON FUNCTION public.list_import_rows_for_commit(BIGINT,UUID,UUID), public.fail_import_row_after_rollback(BIGINT,BIGINT,UUID,UUID,TEXT,TEXT) FROM PUBLIC'
...
-- HF1 operation binding has no direct table or sequence grants.
SELECT 'REVOKE ALL ON FUNCTION public.read_nomenclature_customization_operation(uuid,text), public.write_nomenclature_onboarding_audit(uuid,uuid,uuid,public.nomenclature_onboarding_audit_action,text) FROM PUBLIC, skia_onboarding'
WHERE to_regprocedure('public.read_nomenclature_customization_operation(uuid,text)') IS NOT NULL \gexec
SELECT 'GRANT EXECUTE ON FUNCTION public.read_nomenclature_customization_operation(uuid,text), public.write_nomenclature_onboarding_audit(uuid,uuid,uuid,public.nomenclature_onboarding_audit_action,text) TO skia_runtime'
WHERE to_regprocedure('public.read_nomenclature_customization_operation(uuid,text)') IS NOT NULL \gexec

-- The role artifact runs before/after bootstrap. Apply onboarding table grants
-- only after every required identity table exists.

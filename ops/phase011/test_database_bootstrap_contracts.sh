...
prepare_pre035() {
  ...
  docker exec "$container" sh -ceu \
    "cp -a /repo /repo-pre035; sed -i -e '/035_remove_legacy_rack_authorities.sql/d' -e '/036_nomenclature_v2_foundation.sql/d' /repo-pre035/ops/phase010/bootstrap.manifest"
...
grep -q '^LEDGER_COUNT=28$' <<<"$clean_output"
grep -q '^SCHEMA_HASH=e2e448d243a9c12d067a0fa3bfecb37177a3caedb5f55a2a38ecfe1c37382329$' <<<"$clean_output"
...
grep -q '^SCHEMA_HASH=e2e448d243a9c12d067a0fa3bfecb37177a3caedb5f55a2a38ecfe1c37382329$' <<<"$upgrade_output"
...
grep -q '^SCHEMA_HASH=e2e448d243a9c12d067a0fa3bfecb37177a3caedb5f55a2a38ecfe1c37382329$' <<<"$idempotent_output"

...
func handleImportInventory(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil || session == nil {
		log.Println("ERROR: No session found")
	...
	if err != nil || extractedData == nil || len(extractedData.Items) == 0 {
		...
		extractedData, err = processWithAI(extractedText, assetType, session.TenantID)
		if err != nil {
	...
	// Guardar activos
	err = 	SaveAssetsToDB(importID, session.TenantID, session.UserID, assetType, cleanedItems)
	if err != nil {
	...
	if errors, ok := stats["errors"].([]string); ok && len(errors) > 0 {
		SaveErrorsToDB(importID, errors)
	}
	if warnings, ok := stats["warnings"].([]string); ok && len(warnings) > 0 {
		SaveWarningsToDB(importID, warnings)
	}
...
func handleImportStats(w http.ResponseWriter, r *http.Request) {
	...
	stats, err := GetImportStatsToDB(session.TenantID)
	if err != nil {
...
func handleRecentImports(w http.ResponseWriter, r *http.Request) {
	...
	imports, err := GetRecentImportsToDB(session.TenantID, 10)
	if err != nil {
...
// getSessionFromCookie obtiene la sesión
...
// SessionData estructura para sesión
